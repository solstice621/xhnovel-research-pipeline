...
def test_consumer_layout_import_does_not_modify_export(tmp_path):
    ...
    export_dir = consumer / "imports" / result["export"]["export_id"]
    ...
    catalog = export_dir / "catalog.json"
    catalog.write_bytes((result["work_dir"] / "catalog.json").read_bytes())
    before = dest.read_bytes()
    lock = export_dir / "import.lock.json"
    store_path = result["root_work_dir"] / "objects"
    first = import_export(dest, lock, trusted_catalog_path=catalog, artifact_store_path=store_path)
    mapping = mapping_dir / f"{result['export']['export_id']}.md"
    mapping.write_text("# mapping\nThis file must not rewrite imports/export.json\n", encoding="utf-8")
    second = import_export(dest, lock, trusted_catalog_path=catalog, artifact_store_path=store_path)
