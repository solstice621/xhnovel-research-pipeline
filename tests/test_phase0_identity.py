...
def test_title_author_normalization_uses_existing_title_rules(tmp_path):
    first = _declaration(tmp_path / "book.txt", work=_work(title="《测试仙途》（小说）"))
    second = _declaration(tmp_path / "book.txt", work=_work(title="测试仙途"))
...
def test_source_ref_depends_on_source_config_not_rights_or_brief(tmp_path):
    ...
    declaration_b = _declaration(source, rights={**RIGHTS, "may_export_excerpts": True})
    ref_a = source_ref_from_validated(
        declaration_a,
        _validated(brief_a, declaration_a),
        work_ref_from_declaration(declaration_a),
    )
    ref_b = source_ref_from_validated(
        declaration_b,
        _validated(brief_b, declaration_b),
        work_ref_from_declaration(declaration_b),
    )
    assert ref_a["source_ref_id"] == ref_b["source_ref_id"]
