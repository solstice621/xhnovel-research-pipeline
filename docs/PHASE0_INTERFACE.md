...
| Deterministic source-quality tiers: `COMPLETE+OFFICIAL→A`; `COMPLETE+(PUBLISHED_EDITION\|USER_VERIFIED_COPY\|UNKNOWN\|UNOFFICIAL_COPY)→B`; non-`COMPLETE` → `D` (lead-only). Unproven official status is `UNKNOWN` (eligible when complete); `UNOFFICIAL_COPY` is a positive unauthorized declaration, admitted at Tier B by operator research policy (2026-09, `novel-source-classifier-v3`) and never Tier A. Tier `C` exists only in the separate reviewer path | `novel_assessment.py:184-206`; `390-445` |
...
- **Validator — Handoff-only strictness** — inputs legal under `RUNTIME_COMPAT` but
  ...
  `EVIDENCE_HANDOFF` → REJECT). `UNKNOWN+COMPLETE` and `UNOFFICIAL_COPY+COMPLETE`
  (operator policy, classifier v3) are Tier B and are accepted.
