...
1. `PLACE_MENTION`: a named place that the text itself mentions. Emit only kind and name; this fact says the named place is mentioned.
...
3. `PLACE_TYPE_ASSERTION`: an explicit classification of a named place. Use place_name and explicit_type. Emit its PLACE_MENTION as well. A place mention without a classification needs no type assertion.
...
- Each named place mention payload is emitted once per unit. Each distinct explicit place/type assertion is emitted once separately.
...
- A suffix inside a place name is not by itself a classification assertion. In "他来到了乌坦城", emit only PLACE_MENTION. In "乌坦城是一座城市", the explicit type is "城市".
- Emit neither multiple synonymous types nor an extra type variant from the same mention occurrence. Different occurrences may explicitly assert different types; retain those different assertions with their own evidence.
